  ...
  try {
    ...
    if (provider === "firebase") {
      ...
      if (!resp.ok) {
        ...
        return res
          .status(400)
          .json({ error: "Firebase token refresh failed", details: data })
      }
    ...
    if (provider === "google") {
      ...
      if (!clientId || !clientSecret)
        return res
          .status(500)
          .json({
            error: "Server misconfiguration: GOOGLE_CLIENT_ID/SECRET missing",
          })
      ...
      if (!resp.ok) {
        ...
        return res
          .status(400)
          .json({ error: "Google token refresh failed", details: data })
      }
    ...
    return res
      .status(400)
      .json({ error: 'Unsupported provider. Use "google" or "firebase".' })
  } catch (err) {
